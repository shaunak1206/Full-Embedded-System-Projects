...
// output to ST7735 0.000cm to 2.000cm
  uint32_t ones = n / 1000;             
  uint32_t tenths = (n / 100) % 10;     
  uint32_t hundredths = (n / 10) % 10;  
  uint32_t thousandths = n % 10; 
  
  // ST7735_SetCursor(0, 0);    
  // ST7735_OutChar(ones);
  // ST7735_OutChar('.');
  // ST7735_OutChar(tenths);
  // ST7735_OutChar(hundredths);
  // ST7735_OutChar(thousandths);

  printf("d= %i.%i%i%i cm          ",ones,tenths,hundredths,thousandths);   
...
int main4(void){ // main4
  ...
  PLL_Init(); // set bus speed
  LaunchPad_Init();
  //extern void ST7735_InitPrintf(enum initRFlags option);
